...
opt = {
  ...
  nz = 100,              
  doc_length = 250 ,
  queries = 'scripts/person_queries.txt',
  checkpoint_dir = 'checkpoints',
  net_gen = 'person_nc2_cls0.5_int1.0_ngf128_ndf64_300_net_G.t7',
  net_txt = 'txt_model/model.t7',
}
...
net_txt = torch.load(opt.net_txt):cuda()
...
for query_str in io.lines(opt.queries) do
  local txt = torch.zeros(1,opt.doc_length,#alphabet + 2)
  for t = 1,opt.doc_length do
    local index = 1
    if t > query_str:len() then
       index = #alphabet + 2
    elseif type(dict[query_str:sub(t, t)]) == nil then
       index = #alphabet + 1
    else
       index = dict[query_str:sub(t, t)]
    end
    -- local index = self.dict[self.sentence[i]:sub(j, j)]
    txt[{1, t, index}] = 1
  end
